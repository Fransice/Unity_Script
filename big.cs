           ...
           //如果有两个及以上的手指按下
            if (Input.touchCount > 1)
            {
                ...
                if (isTwoTouch)
                {
                    ...
                    //当前图片的缩放
                    Vector3 curImageScale = new Vector3(image.transform.localScale.x, image.transform.localScale.y, 1);
                    //两根手指上一帧和这帧之间的距离差
                    ...
                    image.transform.localScale = curImageScale + changeScale;
                    //控制缩放级别
                    image.transform.localScale = new Vector3(Mathf.Clamp(image.transform.localScale.x, 0.5f, 10f), Mathf.Clamp(image.transform.localScale.y, 0.5f, 10f), Mathf.Clamp(image.transform.localScale.y, 0.5f, 10f));
